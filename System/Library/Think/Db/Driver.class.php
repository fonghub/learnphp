...
abstract class Driver {
    ...
    public function query($str,$fetchSql=false) {
        try {         
            return $this->doQuery($str,$fetchSql);
        } catch (\PDOException $e) {
     ...
     * @return mixed
     */
    public function doQuery($str,$fetchSql) {
    ...
    public function execute($str,$fetchSql=false) {
        try {         
            return $this->doExecute($str,$fetchSql);
        } catch (\PDOException $e) {
            $this->linkID=array();
            $this->_linkID=null;
            return $this->doExecute($str,$fetchSql);
     ...
     * @return mixed
     */
    public function doExecute($str,$fetchSql) {
    ...
    protected function initConnect($master=true) {
        if(!empty($this->config['deploy']))
            ...
            //连接等待过久放弃使用
            if(time()-$this->lastTime>=$this->config['wait_timeout']){
                $this->cleanLinkID();
            }
            $this->lastTime=time();
            // 默认单数据库
